...
class WeixinController extends Controller{
	...
	function __construct(){
		...
		if(S('access_token')){
			...
			S('access_token',$this->access_token,1600);
		}
	}

    //获取预支付交易会话标识，有效期两个小时
    function get_prepay_id($total_fee,$out_trade_no,$notify_url,$pay_id,$good_name){
...
//        file_put_contents('prepay.txt',$this->appid . ',' . $pay_id . ',' . $good_name . ',' . $this->mch_id . ',' . $nonce_str . ',' . $notify_url . ',' . $out_trade_no . ',' . $total_fee . ',' . $sign);
...
//        file_put_contents('weixin.txt',$result);
	...
	function get_qr_limit($scene_id){
        ...
        $config_info = M('agent_info') -> where("id = {$scene_id}") -> select();
        $this->appid=$config_info[0]['appid'];
        $this->appsecret=$config_info[0]['appsecret'];
        $this->access_token=$this->get_access_token();
		$url="https://api.weixin.qq.com/cgi-bin/qrcode/create?access_token=".$this->access_token;
...
//		file_put_contents('a.txt',$res);
		...
		$surl="https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket=".$ticket;
		$ress=$this->http_request($surl);
//        file_put_contents('b.txt',$ress);

        $file_name = 'Public/shanghu/'.$scene_id.'.jpg';
		file_put_contents('Public/shanghu/'.$scene_id.'.jpg',$ress);
		return $file_name;
	}
	//微信支付加密字符串
...
//		file_put_contents('e4re.txt',$arr);
	...
	function get_prepay_idd($openid,$total_fee,$out_trade_no,$notify_url,$pay_id,$good_name){
		...
		//file_put_contents('cas.txt',$result);
		$postObj = simplexml_load_string($result, 'SimpleXMLElement', LIBXML_NOCDATA);
    ...
    public function createMenu($data,$agent_id){
//        $agent_id = M('agent_token') -> getFieldByAgentToken($agent_id,'agent_id');
        $config_info = M('agent_info') -> where("id = {$agent_id}") -> select();
        $this->appid=$config_info[0]['appid'];
        $this->appsecret=$config_info[0]['appsecret'];
        $this->access_token=$this->get_access_token();
        return $this -> send_menu($data);
	...
	public function send_menu($data){
		...
		$res = $this->http_request($url,$data);
//		file_put_contents('menu.txt',$res);
