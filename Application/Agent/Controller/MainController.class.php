...
class MainController extends ActionController{
	...
	function index(){
		...
		/* 商城成交总额 */
		$shop_all_fee = $shop_order -> where("state = 1 and agent_id = {$this->id}")-> sum('zonge');
...
time,'%Y-%m-%d') = current_date() and agent_id = {$this->id};";
        $model = M();
...
time,'%Y-%m-%d') = current_date() and agent_id = {$this->id};";
		...
		$user_all_tixian = M('shop_order_detail')
            ...
            -> where("state = 1 and agent_id = {$this->id}")
            -> select();
        ...
        $tixian_all_money = M('tixian')
            ...
            -> where("type = 1 and agent_id = {$this->id}")
            -> select();
...
time,'%Y-%m-%d') = current_date() and agent_id = {$this->id};";
        ...
        //钱包余额
        $money = M('user') -> field("sum(money) as a") -> where("agent_id = {$this->id}") -> select();
