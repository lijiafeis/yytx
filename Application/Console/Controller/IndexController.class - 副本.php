        ...
        foreach ($data as $k => $v){
           ...
           // dump($tuijianjiang);
            ...
            if($res){
                ...
                M('yongjin') -> where("id = {$v['id']}") -> setField('time1',time());
...
//        exit;
        ...
        foreach ($data as $k => $v){
			...
			//判断是否超过了
			$zMoney = M('yongjin') -> where("user_id = {$v['user_id']} and is_flag = 0") -> sum('money');
            $fee = M('shop_order_detail') -> where("user_id = {$v['user_id']} and state = 1 and flag = 0") -> sum('good_price');
            if($zMoney > $fee * 2){
...
//        dump($data);
    ...
    private function abc($user_id){
	    ...
	    foreach ($xjuser as $k => $v){
            $number = $model -> getFieldByUserId($v['user_id'],'number');
            $number1 = $model -> getFieldByUserId($v['user_id'],'number1');
