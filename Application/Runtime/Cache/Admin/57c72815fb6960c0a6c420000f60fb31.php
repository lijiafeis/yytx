<?php if (!defined('THINK_PATH')) exit();?><table class="table table-bordered table-hover">
	...
	<?php if(is_array($order_info)): $kk = 0; $__LIST__ = $order_info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vv): $mod = ($kk % 2 );++$kk;?><tr id="<?php echo ($kk); ?>">
		...
		<td width="30%"><?php echo ($vv["address"]["username"]); ?> Tel:<?php echo ($vv["address"]["telphone"]); ?><br /><?php echo ($vv["address"]["city"]); echo ($vv["address"]["address"]); ?></td>
		<td><?php echo ($vv["wxname"]); ?></td>
		<td><?php echo ($vv["zonge"]); ?></td>
		...
		<?php if($vv["state"] == 0): ?><a onclick="confirm(<?php echo ($vv["id"]); ?>)"><button class="btn btn-default btn-sm">确认订单</button></a><?php endif; ?>
		</td>
